...
INSERT INTO vehiculo(registro_unico, marca, modelo, anio, capacidad_carga, consumo_promedio_combustible,numero_placa, fecha_adquisicion, 
km_actual, fecha_soat, fecha_tecnomecanica,estado_operativo, tipo_vehiculo)
VALUES
('C-001','mercedes benz','atego 1726',2018,12,28.2,'FHF388','2023-01-15',320500,'2026-05-10','2026-06-15','activo','camion'),
('C-002','chevrolet','nhr euro',2022,3.2,12.5,'FKX358','2022-05-10',150300,'2025-12-01','2026-01-20','activo','camion'),
('M-001','yamaha','r15 v4',2021,3.0,2.5,'MPU57E','2022-04-06',54500,'2026-03-01','2026-02-15','activo','motocicleta'),
('C-003','volkswagen','worker',2015,18.0,32,'LZR365','2015-07-12',500000,'2024-12-01','2025-01-10','en mantenimiento','camion'),
('F-001','renault','kangoo express',2023,0.75,8.7,'SRL856','2022-05-23',80000,'2026-07-01','2026-07-15','activo','furgoneta'),
('M-002','akt','cr4 200 pro',2022,0.20,3.0,'AMA40G','2020-06-18',150300,'2025-11-20','2025-12-05','en mantenimiento','motocicleta'),
('C-004','mercedes benz','atego 1726',2020,12,28.2,'MDX863','2020-07-15',280420,'2026-04-01','2026-04-10','activo','camion'),
('F-002','nissan','nv200',2019,1.2,9.5,'QZM530','2018-03-02',380000,'2025-09-15','2025-10-01','en mantenimiento','furgoneta'),
('C-005','mercedes benz','atego 1726',2018,12,28.2,'NQD185','2018-05-12',640000,'2026-02-01','2026-02-20','activo','camion'),
('F-003','nissan','nv200',2018,1.2,9.5,'NJD262','2018-05-12',582000,'2026-08-01','2026-08-20','activo','furgoneta');
...
INSERT INTO envios(
numero_guia,fecha_hora_recepcion,id_cliente,id_conductor,id_vehiculo,remitente,destinatario,direccion_origen,
direccion_destino,tipo_servicio,descripcion_contenido,peso,dimensiones,valor_declarado,costo,forma_pago,
estado_actual,instrucciones_especiales,ruta_distribucion,tiempo_estimado_entrega,duracion_estimada_hora)
VALUES
('G-001','2026-01-10 08:30:00',1,1,1,'Carlos Ramírez','Ana Gómez','Calle 10 #20-30, Medellín','Calle 50 #40-25, 
Bogotá','express','Documentos legales',2.50,'30x20x10 cm',500000,25000,'tarjeta','en transito',
'Entregar en portería',1,'2026-01-10 10:30:00',24),
('G-002','2026-01-11 09:15:00',2,2,3,'Textiles Antioquia','Juan Pérez','Carrera 15 #45-60, Cali','Calle 80 #12-34, 
Medellín','normal','Paquete de ropa',5.20,'60x40x30 cm',800000,40000,'efectivo','recibido',NULL,2,
'2026-01-11 12:15:00',72),
...
'transferencia','en transito','Frágil, manejar con cuidado',1,'2026-02-12 15:00:00',6),
('G-004','2026-03-13 10:45:00',1,4,7,'Carlos Ramírez','Empresa ABC','Calle 25 #30-40, Medellín','Calle 70 #15-20, 
Cartagena','express','Papelería corporativa',3.00,'40x30x20 cm',300000,20000,'tarjeta','en transito',NULL,3,
'2026-03-13 12:45:00',24),
...
'en transito','Requiere montacargas',2,'2026-03-14 20:20:00',96),
('G-006','2026-03-15 11:00:00',3,6,9,'María López','Ana Gómez','Av. Libertad 456, Bucaramanga','Calle 40 #25-30, 
Medellín','mismo dia','Flores y regalos',1.50,'25x25x25 cm',200000,15000,'transferencia','entregado',
'Entregar personalmente',1,NULL,5),
('G-007','2024-02-16 13:30:00',4,7,4,'Luisa Sánchez','Pedro Torres','Calle 12 #34-56, Bogotá','Calle 22 #45-67, 
Cali','express','Equipo de oficina',8.00,'70x50x40 cm',1200000,60000,'tarjeta','en transito',NULL,3,
'2024-02-16 15:30:00',24),
...
'recibido','Caja frágil',2,'2026-02-28 12:50:00',72),
...
'en transito','Entrega urgente',3,'2024-02-18 17:10:00',24),
...
'en transito','Mantener refrigerado',1,'2026-03-19 18:25:00',8);
...
-- Eventos:
-- 1. EVT_ActualizarEstimacionesEntrega: Actualiza las estimaciones de entrega según tráfico y retrasos.

DELIMITER $$
CREATE EVENT EVT_ActualizarEstimacionesEntrega
ON SCHEDULE EVERY 5 MINUTE 
STARTS CURRENT_TIMESTAMP
COMMENT 'Actualiza las estimaciones de entrega segun trafico y retrasos'
DO
BEGIN
-- Actualizar estimaciones de entrega para envíos en tránsito
    UPDATE envios e
...
SELECT envioID, estado_actual, tiempo_estimado_entrega
FROM envios
WHERE estado_actual = 'en transito';

SELECT * FROM auditoria_eventos ORDER BY fecha_ejecucion DESC;  

-- 2. EVT_VerificarDocumentacionFlota: Verifica periódicamente la documentación de vehículos.

DELIMITER $$
CREATE EVENT EVT_VerificarDocumentacionFlota
...
-- Registrar ejecución en auditoría
    INSERT INTO auditoria_eventos (nombre_evento, descripcion, fecha_ejecucion)
    VALUES ('EVT_VerificarDocumentacionFlota',
            'Se verifico la documentacion de los vehiculos y se actualizaron estados segun vencimiento',
            NOW());
END$$
DELIMITER ;

-- Auditoría del evento
SELECT nombre_evento, descripcion, fecha_ejecucion
FROM auditoria_eventos
WHERE nombre_evento = 'EVT_VerificarDocumentacionFlota'
ORDER BY fecha_ejecucion DESC;

-- 3. EVT_OptimizarRutasDiarias: Optimiza la asignación de rutas diariamente.
DELIMITER $$

CREATE EVENT EVT_OptimizarRutasDiarias
ON SCHEDULE EVERY 5 MINUTE  -- se asigna cada 5 minutos por temas de pruebas y que usted pueda comprobarlo facil.
STARTS CURRENT_TIMESTAMP
COMMENT 'Optimiza la asignacion de rutas diariamente'
DO
BEGIN
    UPDATE envios e
    JOIN ruta_distribucion r ON e.ruta_distribucion = r.ruta_distribucionID
    JOIN vehiculo v ON e.id_vehiculo = v.vehiculoID
    SET e.ruta_distribucion = r.ruta_distribucionID
    WHERE e.estado_actual = 'en transito'
...
-- Revisar auditoría del evento
SELECT nombre_evento, descripcion, fecha_ejecucion
FROM auditoria_eventos
WHERE nombre_evento = 'EVT_OptimizarRutasDiarias'
ORDER BY fecha_ejecucion DESC;


-- 4. EVT_GenerarReporteFuelConsumption: Genera reportes de consumo de combustible por vehículo.

DELIMITER $$

CREATE EVENT EVT_GenerarReporteCombustion
...
-- Registrar en auditoría un resumen del consumo por vehículo
    ...
    SELECT 'EVT_GenerarReporteCombustion' AS nombre_evento,
           CONCAT('Vehículo ', c.vehiculo,
                  ' consumió en promedio ',
                  ROUND(AVG(c.cantidad),2), ' litros de ',
                  GROUP_CONCAT(DISTINCT c.tipo_combustible SEPARATOR '/'),
                  ' - Km actual: ', MAX(c.km_momento_carga)) AS descripcion,
...
SELECT nombre_evento, descripcion, fecha_ejecucion
FROM auditoria_eventos
WHERE nombre_evento = 'EVT_GenerarReporteCombustion'
ORDER BY fecha_ejecucion DESC;


-- 5. EVT_MonitorearDesviacionesRuta: Detecta desviaciones significativas en las rutas asignadas.

DELIMITER $$

CREATE EVENT EVT_MonitorearDesviacionesRuta
ON SCHEDULE EVERY 30 MINUTE -- este tiempo sirve para detectar una desviacion significativa, una hora es mucho 
STARTS CURRENT_TIMESTAMP
...
-- monitorea los envios en transito 
    ...
    WHERE e.estado_actual = 'en transito'
      AND EXISTS (
          SELECT 1
          FROM seguimiento_ruta s
          WHERE s.envioID = e.envioID
            AND s.desviacion_detectada = 1
      );

-- Registrar seguimiento en auditoría
    INSERT INTO auditoria_eventos (nombre_evento, descripcion, fecha_ejecucion)
    VALUES ('EVT_MonitorearDesviacionesRuta',
            'Se monitorearon envíos en tránsito y se marcaron desviaciones detectadas',
            NOW());
END$$
...
SELECT nombre_evento, descripcion, fecha_ejecucion
FROM auditoria_eventos
WHERE nombre_evento = 'EVT_MonitorearDesviacionesRuta'
ORDER BY fecha_ejecucion DESC;


-- Revisar todas las auditorias
SELECT * FROM auditoria_eventos ORDER BY fecha_ejecucion DESC;  

-- si queremos apagar todos los eventos:
SET GLOBAL event_scheduler = OFF;

-- si queremos apagar un evento en particular:
ALTER EVENT EVT_ActualizarEstimacionesEntrega DISABLE;
ALTER EVENT EVT_VerificarDocumentacionFlota DISABLE;
ALTER EVENT EVT_OptimizarRutasDiarias DISABLE;
ALTER EVENT EVT_GenerarReportecombustion DISABLE;
ALTER EVENT EVT_MonitorearDesviacionesRuta DISABLE;
